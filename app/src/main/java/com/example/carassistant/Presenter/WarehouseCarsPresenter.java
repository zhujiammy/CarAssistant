...
import com.example.carassistant.View.WarehouseCarsDetailsActivity;
...
public class WarehouseCarsPresenter implements WarehouseCarsInterface.Presenter {
    public WarehouseCarsInterface.View view;
    public Context context;
    JsonArray jsonElements;
    private WarehouseCarsAdapter adapter;
    RecyclerViewEmptySupport recyclerView;
    public WarehouseCarsPresenter(WarehouseCarsInterface.View view,Context context,RecyclerViewEmptySupport recyclerView,WarehouseCarsAdapter adapter){
        this.view = view;
    ...
    @Override
    public void getWarehouseCars(String searchInfo) {
        Call<ResponseBody> call = HttpHelper.getInstance().create(CarAssistantAPI.class).getWarehouseCars(Utils.getShared2(context,"token"),searchInfo);
                ...
                if(response.body()!=null){
                    try {
                        ...
                        if(jsonObject.get("status").getAsInt() == 0){
                            ...
                            adapter = new WarehouseCarsAdapter(context,jsonElements,2);
                            adapter.setOnitemClickListener(new WarehouseCarsAdapter.OnitemClickListener() {
                                @Override
                                public void onItemClick(View view, int position) {
                                    JsonObject object =jsonElements.get(position).getAsJsonObject();
                                    Intent intent = new Intent(context, WarehouseCarsDetailsActivity.class);
                                    ...
                                    if(!object.get("dismantleType").isJsonNull()){
                                        intent.putExtra("dismantleType",object.get("dismantleType").getAsString());
                                    }else {
                                        intent.putExtra("dismantleType","无");
                                    }
