...
import com.example.carassistant.Interface.VehiclesDismantledInterface;
...
import com.example.carassistant.View.DismantleConfirmCarDetailActivity;
...
import com.example.carassistant.adapter.VehiclesDismantledAdapter;
...
public class VehiclesDismantledPresenter implements VehiclesDismantledInterface.Presenter {
    public VehiclesDismantledInterface.View view;
    public Context context;
    JsonArray jsonElements;
    private VehiclesDismantledAdapter adapter;
    RecyclerViewEmptySupport recyclerView;
    public VehiclesDismantledPresenter(VehiclesDismantledInterface.View view,Context context,RecyclerViewEmptySupport recyclerView,VehiclesDismantledAdapter adapter){
        this.view = view;
    ...
    @Override
    public void searchDismantleConfirm(String searchInfo) {
        Call<ResponseBody> call = HttpHelper.getInstance().create(CarAssistantAPI.class).searchDismantleConfirm(Utils.getShared2(context,"token"),searchInfo);
                ...
                if(response.body()!=null){
                    try {
                        ...
                        if(jsonObject.get("status").getAsInt() == 0){
                            ...
                            adapter = new VehiclesDismantledAdapter(context,jsonElements);
                            adapter.setOnitemClickListener(new VehiclesDismantledAdapter.OnitemClickListener() {
                                @Override
                                public void onItemClick(View view, int position) {
                                    JsonObject object =jsonElements.get(position).getAsJsonObject();
                                    Intent intent = new Intent(context, DismantleConfirmCarDetailActivity.class);
