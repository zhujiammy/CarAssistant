...
import com.example.carassistant.Interface.TestVehicleDestroyedInterface;
...
import com.example.carassistant.View.TestVehicleDestroyeDetailActivity;
...
import com.example.carassistant.adapter.TestVehicleDestroyedAdapter;
...
public class TestVehicleDestroyedPresenter implements TestVehicleDestroyedInterface.Presenter {
    public TestVehicleDestroyedInterface.View view;
    public Context context;
    JsonArray jsonElements;
    private TestVehicleDestroyedAdapter adapter;
    RecyclerViewEmptySupport recyclerView;
    public TestVehicleDestroyedPresenter(TestVehicleDestroyedInterface.View view,Context context,RecyclerViewEmptySupport recyclerView,TestVehicleDestroyedAdapter adapter){
        this.view = view;
    ...
    @Override
    public void testcarsearchDestroyCars(String searchInfo) {
        Call<ResponseBody> call = HttpHelper.getInstance().create(CarAssistantAPI.class).testcarsearchDestroyCars(Utils.getShared2(context,"token"),searchInfo);
                ...
                if(response.body()!=null){
                    try {
                        ...
                        if(jsonObject.get("status").getAsInt() == 0){
                            ...
                            adapter = new TestVehicleDestroyedAdapter(context,jsonElements);
                            adapter.setOnitemClickListener(new TestVehicleDestroyedAdapter.OnitemClickListener() {
                                @Override
                                public void onItemClick(View view, int position) {
                                    JsonObject object =jsonElements.get(position).getAsJsonObject();
                                    Intent intent = new Intent(context, TestVehicleDestroyeDetailActivity.class);
