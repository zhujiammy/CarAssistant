...
//车辆出库
public class VehicleOutboundActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.vehicleoutbound);
    ...
    private void getChooseDismantleTypeCarsDetails(){
        Call<ResponseBody> call = HttpHelper.getInstance().create(CarAssistantAPI.class).searchCarsDetails(Utils.getShared2(getApplicationContext(),"token"),getIntent().getStringExtra("carDetailId"));
    ...
    private void testcarsearchCarsDetails(){
        Call<ResponseBody> call = HttpHelper.getInstance().create(CarAssistantAPI.class).testcarsearchCarsDetails(Utils.getShared2(getApplicationContext(),"token"),getIntent().getStringExtra("carDetailId"));
        call.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                if(response.body()!=null){
    ...
    private void showdilgo(){
        final AlertDialog.Builder alterDiaglog = new AlertDialog.Builder(VehicleOutboundActivity.this);
        alterDiaglog.setTitle("提示");//文字
        alterDiaglog.setMessage("确定出库吗？");//提示消息
        alterDiaglog.setPositiveButton("确定", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                progressDialog.show();
                if(getIntent().getStringExtra("type").equals("1")){
                    Call<ResponseBody> call = HttpHelper.getInstance().create(CarAssistantAPI.class).EXWarehouseCar(Utils.getShared2(getApplicationContext(),"token"),getIntent().getStringExtra("carDetailId"),getIntent().getStringExtra("flowId"));
                    call.enqueue(new Callback<ResponseBody>() {
                        @Override
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                            if(response.body()!=null){
                    ...
                    Call<ResponseBody> call = HttpHelper.getInstance().create(CarAssistantAPI.class).testcarEXWarehouseCar(Utils.getShared2(getApplicationContext(),"token"),getIntent().getStringExtra("carDetailId"));
                    call.enqueue(new Callback<ResponseBody>() {
                        @Override
                        public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                            if(response.body()!=null){
